...
var handbagsAndMoreWomen = []; // Bracelets (BRACELET , WATCH)
...
$w.onReady(function () {
    ...
    setTimeout(() => {
        ...
        dbQueryAllResponseItems.forEach((item) => {
            ...
            // current product's external link
            var currentProductsLink = decodeCodifiedUrl(item.productLink);
            var currentProductLinkIncludesAmazon = currentProductsLink.includes('amazon');
            ...
            if (item.gender == 'WOMEN' && gallery1AlreadyIncludedAsPerTheirRelativeGenderTrackerList) {
                if (typeof (totalProductsPerCategoryPerGender[productGender][productCat]) != 'object') {
                    ...
                    // INCLUDING ONLY AMAZON PRODUCTS
                    if (currentProductLinkIncludesAmazon){

                        // console.log('is amazon product ' + currentProductLinkIncludesAmazon,  currentProductsLink);
                        totalProductsPerCategoryPerGender.WOMEN[item.productCategory] = [item];
            ...
            for (var gender in totalProductsPerCategoryPerGender) {
                ...
                for (var productCategory in totalProductsPerCategoryPerGender[gender]) {
                    ...
                    if (totalProductsPerCategoryPerGender[gender][productCategory].length != 0) {
                        ...
                        var currentProductsLink = decodeCodifiedUrl(currentProduct.productLink);

                        var linkIncludesAmazon = currentProductsLink.includes('amazon');

                        //if (linkIncludesAmazon){
                        //
                        //    console.log('PRODUCT LINK -> ' + linkIncludesAmazon, currentProduct.productCategory);
                        //
                        //}
                        // console.log('PRODUCT LINK -> ' + currentProductsLink.includes('amazon'), currentProduct.productCategory);

                        // console.log('currentProduct: ' + currentProduct.title);

                        if (currentProduct.siteName != ''  && (currentProduct.productCategory.includes('ACCESSORIES') ||
                                currentProduct.productCategory.includes('ACCESSORY') || currentProduct.productCategory.includes('RING') ||
                                currentProduct.productCategory.includes('NECKLACE') || currentProduct.productCategory.includes('EARRING') ||
                                currentProduct.productCategory.includes('BRACELET'))) {



                            // console.log(currentProduct.productCategory, currentProduct.title);
        ...
        if (ladiesShoesIncludedCount < 3 ||
            ...
            if (typeof (womensFeaturableShoesListCopy) == 'object' && dbQueryAllResponseItems.length == 0) {
        ...
        if (isDisplayShoesAndMore) {
            listOfProductCategoryForWomen.push({ "label": "Necklaces & Earrings", "value": "Necklaces & Earrings" });
        }

        if (isDisplayHandbagsAndMore) {
            listOfProductCategoryForWomen.push({ "label": "Bracelets", "value": "Bracelets" });
        }
        ...
        if (usersCountryCode != 'US'){
            if (isDisplayFlowersAndWomensRings) {
            listOfProductCategoryForWomen.push({ "label": "Rings", "value": "Rings" });
            }
        }
        ...
        //     listOfProductCategoryForMen.push({ "label": "Rings", "value": "Rings" });
        // }
        ...
        $w('#dropdown1').onChange((event) => {
            ...
            if ($w('#dropdown1').value == 'Women') {
                $w('#dropdown3').options = listOfProductCategoryForWomen;
                $w('#dropdown3').value = 'Necklaces & Earrings';
                ...
                $w('#dropdown3').value = 'Necklaces & Bracelets';
...
function setMainGalleryBasedOnProductCategory(
    ...
    if (isRecommendedGalleryForDesktop == true) {
        ...
        if (gender == 'Women') {
            ...
            // product categories that pertain to women
            else if (currentProductCategory == 'Bracelets') {
