...
def filter_watches_com_scraped_data(
    ...
    try:
        watches_com_scrapped_data = watches_com_scrapped_data[
            ['productLink-href', 'productImage', 'productName', 'currentPriceOriginalPrice']
        ] # !!
    except:
    ...
    watches_com_scrapped_data = watches_com_scrapped_data[starting_index:]

    # CLEANING UP
    print()
    product_link = watches_com_scrapped_data['productLink-href']
    image_link = watches_com_scrapped_data['productImage']
    product_name = watches_com_scrapped_data['productName']
    current_price = watches_com_scrapped_data['currentPriceOriginalPrice']
    # print(product_link)
    ...
    for productLink, imageLink in zip(product_link, image_link):
        ...
        # 2.a
            ...
            # remove_row = watches_com_scrapped_data.drop(axis=0, index=countLinkNumber, inplace=True)
            # print(remove_row)
    ...
    # watches_com_scrapped_data['currentPrice'] = current_price

    watches_com_scrapped_data = watches_com_scrapped_data[
        ['productLink-href', 'productImage', 'productName', 'currentPriceOriginalPrice']
    ]

    watches_com_scrapped_data.columns = \
        ['productLink', 'Image Src', 'Title', 'Price']

    watches_com_scrapped_data['productLink'] = product_link
    watches_com_scrapped_data['Image Src'] = image_link
    watches_com_scrapped_data['Title'] = product_name
    watches_com_scrapped_data['Price'] = current_price

    # DROPPING ALL PRODUCTS THAT DO HAVE a price that's lower than the price required to hit 'minimum_profit_target'
    watches_com_scrapped_data.dropna(inplace=True)
    watches_com_scrapped_data.reset_index(drop=True, inplace= True)

    cleaned_up_scraped_data_watches_com = watches_com_scrapped_data

    print(cleaned_up_scraped_data_watches_com)

    # cleaned_up_scraped_data_watches_com['currentPrice'] = [float(i.split(" ")[-1][1:].replace(",", "")) for i in
    #                                        cleaned_up_scraped_data_watches_com['currentPrice']]
#
    # print()

    # GROUPING CLEANUP_UP watches_com DATA WITH BRAND NAME AND CURRENTPRICE
    # grouped_data = cleaned_up_scraped_data_watches_com[['productName', 'brandName', 'currentPrice']]. \
    ...
    len_after_filtering = len(cleaned_up_scraped_data_watches_com.index)
    ...
    cleaned_up_scraped_data_watches_com.to_csv(f'{all_filtered_data_folder}{file_name[:-4]}_FILTERED.csv', index=False)
    ...
    if len_after_filtering > 0 and is_wx_upload == True:
        ...
        extract_elements_per_row_from_dataframe(
            ...
            dataframe=cleaned_up_scraped_data_watches_com,
